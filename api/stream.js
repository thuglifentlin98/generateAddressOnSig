...
const { map, mergeMap, catchError, toArray, filter } = require('rxjs/operators');
...
async function processAddressesForAllBipTypes(root, network, electrumClient) {
    ...
    const bipTypes = Object.entries(paths).map(([bipType, path]) => {
        return from(processAddresses(root, network, electrumClient, bipType, path)).pipe(
            map(({ usedAddresses, freshReceiveAddress, freshChangeAddress, totalBalance: typeBalance, utxos }) => {
    ...
    await Promise.all(bipTypes.map(bt => bt.toPromise()));
...
async function processAddresses(root, network, electrumClient, bipType, path) {
    ...
    const addresses$ = from(Array(maxIndex).keys()).pipe(
        mergeMap(start => checkAndGenerateAddresses(account, network, bipType, electrumClient, start, batchSize)),
        mergeMap(batchResults => {
            ...
            return of(batchResults);
        }),
        toArray()
    );

    await addresses$.toPromise();
